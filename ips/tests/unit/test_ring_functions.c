...
int main(void) {
    ...
    rc = packet_ring_enq(&ring, (const uint8_t *)"A", 1, 101);
    EXPECT_INT_EQ("packet_ring_enq", 0, rc);
    rc = packet_ring_enq(&ring, (const uint8_t *)"B", 1, 202);
    EXPECT_INT_EQ("packet_ring_enq", 0, rc);
    rc = packet_ring_enq(&ring, (const uint8_t *)"C", 1, 303);
    EXPECT_INT_EQ("packet_ring_enq", -1, rc);

    rc = packet_ring_deq(&ring, out, sizeof(out), &out_len, &session_id);
    EXPECT_INT_EQ("packet_ring_deq", 0, rc);
    EXPECT_TRUE("packet_ring_deq", "out_len=1 and first byte=A",
                1 == out_len && 'A' == out[0]);
    EXPECT_INT_EQ("packet_ring_deq.session_id", 101, session_id);

    rc = packet_ring_deq(&ring, out, 0, &out_len, &session_id);
    ...
    EXPECT_TRUE("packet_ring_deq", "out_len=1 and first byte=B",
                1 == out_len && 'B' == out[0]);
    EXPECT_INT_EQ("packet_ring_deq.session_id", 202, session_id);
