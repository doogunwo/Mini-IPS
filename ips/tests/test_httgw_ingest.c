...
static int test_ingest_single_request_with_duplicate_segment(void) {
    ...
    fprintf(stderr,
            "[test_httgw_ingest] case=duplicate_segment req_count=%d "
            "err_count=%d uri=%s final_seq=%u\n",
            ctx.req_count, ctx.err_count, ctx.last_uri,
            seq0 + (uint32_t)strlen(p1));
