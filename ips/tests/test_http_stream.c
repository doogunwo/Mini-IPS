...
static int test_content_length_request(void)
{
    ...
    CHECK(http_stream_feed(s, (const uint8_t *)req, strlen(req)) == HTTP_STREAM_OK,
...
static int test_malformed_header_and_reset(void)
{
    ...
    CHECK(http_stream_feed(s, (const uint8_t *)good, strlen(good)) == HTTP_STREAM_OK,
