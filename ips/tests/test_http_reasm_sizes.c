...
static const size_t g_segment_sizes[] = {
    256U,
    1024U,
    1460U
};
                          ...
                          void *user)
{
    ...
    snprintf(ctx->first_method, sizeof(ctx->first_method), "%.15s", msg->method);
}
...
static int build_tcp_packet(const tcp_pkt_spec_t *sp,
                            uint8_t **out_pkt,
                            uint32_t *out_len)
{
    ...
    total = sizeof(struct ether_header) + IP_HDR_SIZE +
            TCP_HDR_SIZE + sp->payload_len;
    ...
    IP_TOTLEN(ip) = htons((uint16_t)(IP_HDR_SIZE + TCP_HDR_SIZE +
                                     sp->payload_len));
...
static void init_gateway(httgw_cfg_t *cfg,
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(g_uri_sizes) / sizeof(g_uri_sizes[0]); i++) {
        for (j = 0; j < sizeof(g_segment_sizes) / sizeof(g_segment_sizes[0]); j++) {
            if (run_reasm_case(g_uri_sizes[i], g_segment_sizes[j]) != 0) {
                fprintf(stderr,
                        "failed: uri=%zu seg=%zu\n",
                        g_uri_sizes[i],
                        g_segment_sizes[j]);
                return 1;
            }
    ...
    printf("ok: test_http_reasm_sizes\n");
    return 0;
}
