...
static uint16_t tcp_checksum_test(uint32_t src_be, uint32_t dst_be, const uint8_t *tcp, size_t tcp_len)
{
    uint32_t sum = 0;
    const uint8_t *p;
    size_t len;
...
static int assert_rst_pkt(
    const rst_capture_t *cap,
    uint32_t exp_sip,
    uint16_t exp_sport,
    uint32_t exp_dip,
    uint16_t exp_dport,
    ...
    uint32_t exp_ack)
{
    ...
    CHECK(cap->len == IP_HDR_SIZE + TCP_HDR_SIZE, "captured RST length mismatch");
    ...
    IP_CHECK(&ip_copy) = 0;
    CHECK(checksum16_test(&ip_copy, sizeof(ip_copy)) == IP_CHECK(ip), "IP checksum mismatch");

    memcpy(&tcp_copy, tcp, sizeof(tcp_copy));
    TCP_CHECK(&tcp_copy) = 0;
    CHECK(htons(tcp_checksum_test(IP_SADDR(ip), IP_DADDR(ip), (const uint8_t *)&tcp_copy, sizeof(tcp_copy))) == TCP_CHECK(tcp),
...
static int test_rst_seq_ack_bidir(void)
{
    ...
    CHECK(build_tcp_packet(pkt, sizeof(pkt), 0x0A000001, 1111, 0x0A000002, 80,
                           100, 700, TCP_ACK | TCP_PSH, p1, sizeof(p1) - 1, &pkt_len) == 0,
          "build AB packet failed");
    CHECK(httgw_ingest_packet(gw, pkt, pkt_len, 1) == 1, "ingest AB packet failed");

    CHECK(build_tcp_packet(pkt, sizeof(pkt), 0x0A000002, 80, 0x0A000001, 1111,
                           700, 110, TCP_ACK | TCP_PSH, p2, sizeof(p2) - 1, &pkt_len) == 0,
          "build BA packet failed");
    CHECK(httgw_ingest_packet(gw, pkt, pkt_len, 2) == 1, "ingest BA packet failed");
    ...
    cap.calls = 0;
    CHECK(httgw_request_rst(gw, &flow, DIR_AB) == 0, "httgw_request_rst AB failed");
    CHECK(assert_rst_pkt(&cap, 0x0A000001, 1111, 0x0A000002, 80, 110 + 4095, 767) == 0,
          "AB RST packet assert failed");

    cap.calls = 0;
    CHECK(httgw_request_rst(gw, &flow, DIR_BA) == 0, "httgw_request_rst BA failed");
    CHECK(assert_rst_pkt(&cap, 0x0A000002, 80, 0x0A000001, 1111, 767 + 4095, 110) == 0,
...
static int test_rst_fallback_uses_other_direction(void)
{
    ...
    CHECK(build_tcp_packet(pkt, sizeof(pkt), 0x0A000001, 3333, 0x0A000002, 8080,
                           100, 50, TCP_ACK | TCP_PSH, payload, sizeof(payload) - 1, &pkt_len) == 0,
          "build AB packet failed");
    CHECK(httgw_ingest_packet(gw, pkt, pkt_len, 5) == 1, "ingest AB packet failed");
    ...
    CHECK(httgw_request_rst(gw, &flow, DIR_BA) == -3, "httgw_request_rst BA fallback failed");
