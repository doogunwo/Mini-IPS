...
static void init_common(httgw_cfg_t *cfg, httgw_callbacks_t *cbs, test_ctx_t *ctx)
{
...
static int test_header_split_across_segments(void)
{
    ...
    CHECK(ctx.req_count == 0, "request must not flush before header terminator completes");
...
static int test_cl_te_conflict(void)
{
    ...
    CHECK(ctx.req_count == 0, "CL+TE conflict should not produce request callback");
...
static int test_out_of_order(void)
{
    ...
    CHECK(ctx.req_count == 1, "expected exactly one request_out_of_order"); // "late-start reasm should not recover missing prefix"
    CHECK(strcmp(ctx.last_uri, "/a") == 0, "uri mismatch");
    CHECK(ctx.err_count == 0, "unexpected error callback");
