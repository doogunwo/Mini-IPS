...
char *app_render_block_page(const char *template_path,
                            const char *event_id,
                            ...
                            const char *client_ip)
{
    ...
    while (*p != '\0')
    {
        ...
        {
            if (html_buf_append_html_escaped(&out, event_id ? event_id : "-") != 0)
            {
                free(template_text);
                free(out.buf);
                return NULL;
        ...
        {
            if (html_buf_append_html_escaped(&out, timestamp ? timestamp : "-") != 0)
            {
                free(template_text);
                free(out.buf);
                return NULL;
        ...
        {
            if (html_buf_append_html_escaped(&out, client_ip ? client_ip : "-") != 0)
            {
                free(template_text);
                free(out.buf);
                return NULL;
