...
setup_sysctl() {
  echo "Applying sysctl settings inside the namespace..."
  
  # 검사만 하지 말고 직접 써버립니다 (set)
  sysctl -w net.bridge.bridge-nf-call-iptables=1 || true
  sysctl -w net.ipv4.conf.all.route_localnet=1
  sysctl -w net.ipv4.conf.all.rp_filter=0
  sysctl -w net.ipv4.conf.default.rp_filter=0
  
  # 인터페이스별로도 확실하게 꺼줍니다 (PORT_A, PORT_B 감지 후 실행됨)
  if [[ -n "${PORT_A:-}" ]]; then sysctl -w net.ipv4.conf."${PORT_A}".rp_filter=0; fi
  if [[ -n "${PORT_B:-}" ]]; then sysctl -w net.ipv4.conf."${PORT_B}".rp_filter=0; fi
