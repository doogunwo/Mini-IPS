...
static char *build_http_request(size_t uri_pad_len, size_t *out_len) {
    ...
    const char *hdr_b =
        " HTTP/1.1\r\n"
        "Host: localhost\r\n"
        "User-Agent: detect-pipeline-bench\r\n"
        "Accept: */*\r\n"
...
static int feed_request(httgw_t *gw, const uint8_t *request, size_t request_len,
...
int main(int argc, char **argv) {
    ...
    for (uint32_t i = 0; i < request_count; i++) {
        if (feed_request(gw, (const uint8_t *)request, request_len,
