...
typedef enum {
    HTTGW_MODE_SNIFF = 0,
    HTTGW_MODE_TP_SYN = 1,
    HTTGW_MODE_REVERSE = 2
} httgw_mode_t;


/** 파싱된 클라이언트 요청을 전달하는 콜백이다. */
typedef void (*httgw_on_request_cb)(
...
typedef struct httgw_tp_ctx httgw_tp_ctx_t;
int httgw_tp_mode_init(httgw_t *gw, const httgw_cfg_t *cfg, httgw_tp_ctx_t **out_ctx);
void httgw_tp_mode_destroy(httgw_tp_ctx_t *ctx);

/* Reverse Proxy (skeleton) */
typedef struct httgw_reverse_ctx httgw_reverse_ctx_t;
int httgw_reverse_mode_init(httgw_t *gw, const httgw_cfg_t *cfg, httgw_reverse_ctx_t **out_ctx);
void httgw_reverse_mode_destroy(httgw_reverse_ctx_t *ctx);
